...
class EvaluacionesController {
    ...
    getMultiples(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const arreglo = yield pool.query('SELECT * FROM proyecto.pregunta inner join evaluacion on cod_evaluacion=pregunta.cod_evaluacion_fk where cod_evaluacion_fk=?', [id]);
            if (arreglo.length > 0) {
                return res.json(arreglo);
            }
            else {
