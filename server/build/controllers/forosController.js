...
class ForosController {
    ...
    listPadres(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const arreglo = yield pool.query("Select cod_publicacion, comentario, ref_publi, creador_pu_usuario, cod_foro_fk, usuario.nombre, usuario.cod_usuario, rol.nombre as rol from publicacion inner join usuario on creador_pu_usuario = usuario.cod_usuario inner join rol on rol.cod_rol=usuario.cod_rol_fk where publicacion.ref_publi is null and cod_foro_fk=? order by cod_publicacion desc", [id]);
            res.json(arreglo);
        });
    }
