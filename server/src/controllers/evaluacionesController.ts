...
class EvaluacionesController{
    ...
    //// Crear Preguntas
        ...
        const {id} =req.params;
        const arreglo = await pool.query('SELECT * FROM proyecto.pregunta inner join evaluacion on cod_evaluacion=pregunta.cod_evaluacion_fk where cod_evaluacion_fk=?', [id]);     
        if(arreglo.length>0){
        return res.json(arreglo);
        }else{
